...
bool BlackfinInstruction::DisassemblePushPop(uint16_t instructionWord, struct blackfin::Instruction &instr, bool parallel) {
    ...
    if (!W && mostreg(reg, grp)) {
        ...
        instr.operation = blackfin::OP_POP;
    ...
    // PUSH
        ...
        instr.operation = blackfin::OP_PUSH;
    } else {
        return false;
    }
    return true;
